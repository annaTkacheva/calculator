...
function operationPress(op) {
  ...
  if (MemoryNewNumber && MemoryPendingOperation !== '=') {
    ...
    display.value = Math.round( MemoryCurrentNumber * 10 ) / 10;
...
function root(argument) {
  ...
  if(Math.sign(Number(localRootMemory))=== -1){
    display.value = 'ERROR';
  }else{
    localRootMemory = Math.sqrt(localRootMemory) ;
    display.value = localRootMemory;
  }}
  

function negative(argument) {
  let localRootMemory = display.value;
  localRootMemory = localRootMemory * (-1) ;
  display.value = localRootMemory;
}
